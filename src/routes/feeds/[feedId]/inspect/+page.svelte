<script lang="ts">
	...
	const feedId = $derived(page.params.feedId ?? '');
	...
	$effect(() => {
		const activeFeedId = inspectorState.activeFeedId ?? null;

		if (activeFeedId !== lastFeedId) {
			lastFeedId = activeFeedId;
			treeQuery = '';
			expandedIds.clear();
			selectedNodeId = null;
		}
							...
							<div class="space-y-3 text-sm">
								...
								{#if selectedNode.textPreview}
									<div>
										...
										<div class="mt-2 rounded-md bg-surface-active px-2 py-2 text-xs text-fg-subtle">
